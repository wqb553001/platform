...
import com.doctor.assistant.experiment.czy.poi.excel.entity.result.ExcelImportResult;
import com.doctor.assistant.experiment.czy.poi.excel.entity.result.ExcelVerifyHanlderResult;
import com.doctor.assistant.experiment.czy.poi.excel.imports.verifys.VerifyHandlerServer;
import com.doctor.assistant.experiment.czy.poi.excel.utils.ExcelUtil;
...
public class ExcelImportServer extends ImportBaseService {
     ...
     * @param params
     */
    private Collection addListContinue(Object object, ExcelCollectionParams param, Row row, Map<Integer, String> titlemap, String targetId, Map<String, PictureData> pictures, ImportParams params) throws Exception {
        ...
        for (int i = row.getFirstCellNum(); i < row.getLastCellNum(); i++) {
            ...
            if (param.getExcelParams().containsKey(titleString)) {
                Object value = cellValueServer.getValue(params.getDataHanlder(), object, cell, param.getExcelParams(), titleString);
                if (value != null){
        ...
        if (isUsed) {
            for (ExcelCollectionParams childrenParam : param.getChildrenExcelCollection()) {
                addListContinue(entity, childrenParam, row, titlemap, targetId, pictures, params);
            }
            collection.add(entity);
        } else if (param.getChildrenExcelCollection() != null && param.getChildrenExcelCollection().size() > 0) {
        ...
        while (rows.hasNext() && (row == null || sheet.getLastRowNum() - row.getRowNum() > params.getLastOfInvalidRow())) {
            ...
            if ((row.getCell(params.getKeyIndex()) == null || StringUtils.isEmpty(getKeyValue(row.getCell(params.getKeyIndex()))))
                ...
                try {
                    ...
                    for (ExcelCollectionParams param : excelCollection) {
                        Collection childrenCollection = (Collection) PoiPublicUtil.getMethod(param.getName(), object.getClass()).invoke(object, new Object[]{});
                        Object object_1 = PoiPublicUtil.createObject(pojoClass, targetId);
                        childrenCollection.addAll(addListContinue(object_1, param, row, titlemap, targetId, pictures, params));
                    }
    ...
    private Map<Integer, String> getTitleMap(Sheet sheet, Iterator<Row> rows, ImportParams params, List<ExcelCollectionParams> excelCollection) {
        ...
        //设置表头行数
        int i = ExcelUtil.getHeadRows(sheet, headRow.getRowNum());
        params.setHeadRows(i);
