...
import com.doctor.assistant.experiment.czy.poi.excel.annotation.Excel;
import com.doctor.assistant.experiment.czy.poi.excel.annotation.ExcelCollection;
import com.doctor.assistant.experiment.czy.poi.excel.annotation.ExcelVerify;
import com.doctor.assistant.experiment.czy.poi.excel.entity.params.ExcelCollectionParams;
import com.doctor.assistant.experiment.czy.poi.excel.entity.params.ExcelImportEntity;
import com.doctor.assistant.experiment.czy.poi.excel.entity.params.ExcelVerifyEntity;
import com.doctor.assistant.experiment.czy.poi.excel.entity.params.ImportParams;
import com.doctor.assistant.experiment.czy.poi.excel.utils.ApplicationContextUtil;
    ...
    public void getAllExcelField(String targetId, Field[] fields, Map<String, ExcelImportEntity> excelParams, List<ExcelCollectionParams> excelCollection, Class<?> pojoClass, List<Method> getMethods) throws Exception {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (PoiPublicUtil.isCollection(field.getType())) {
                // 集合对象设置属性
                getExcelFieldList(targetId, field, excelCollection);
...
//                excelCollection.add(collection);
    ...
    private void getExcelFieldList(String targetId, Field field, List<ExcelCollectionParams> excelCollection) throws Exception {
        // 集合对象设置属性
        ExcelCollectionParams collection = new ExcelCollectionParams();
        collection.setName(field.getName());
        Map<String, ExcelImportEntity> temp = new HashMap<String, ExcelImportEntity>();
        ...
        Class<?> clz = (Class<?>) pt.getActualTypeArguments()[0];
        collection.setType(clz);
        getExcelFieldList(targetId, PoiPublicUtil.getClassFields(clz), clz, temp, null, collection);
        collection.setExcelParams(temp);
        collection.setExcelName(field.getAnnotation(ExcelCollection.class).name());
        additionalCollectionName(collection);
        excelCollection.add(collection);
    ...
    public void getExcelFieldList(String targetId, Field[] fields, Class<?> pojoClass, Map<String, ExcelImportEntity> temp, List<Method> getMethods, ExcelCollectionParams collection) throws Exception {
        ExcelImportEntity excelEntity = null;
        List<ExcelCollectionParams> childrenExcelCollection = new ArrayList<>();
        for (int i = 0; i < fields.length; i++) {
            Field field = fields[i];
            if (PoiPublicUtil.isNotUserExcelUserThis(null, field, targetId)) {
                continue;
            ...
            if (PoiPublicUtil.isJavaClass(field)) {
                ...
                ExcelCollectionParams childrenCollection = new ExcelCollectionParams();
                childrenCollection.setName(field.getName());
                Map<String, ExcelImportEntity> childrenTemp = new HashMap<String, ExcelImportEntity>();
                ParameterizedType pt = (ParameterizedType) field.getGenericType();
                Class<?> clz = (Class<?>) pt.getActualTypeArguments()[0];
                childrenCollection.setType(clz);
...
//                newMethods.add(PoiPublicUtil.getMethod(field.getName(), pojoClass, field.getType()));
                getExcelFieldList(targetId, PoiPublicUtil.getClassFields(clz), clz, childrenTemp, newMethods, childrenCollection);

                childrenCollection.setExcelParams(childrenTemp);
                childrenCollection.setExcelName(field.getAnnotation(ExcelCollection.class).name());
                additionalCollectionName(childrenCollection);
                childrenExcelCollection.add(childrenCollection);
            }
        }
        collection.setChildrenExcelCollection(childrenExcelCollection);
    }
